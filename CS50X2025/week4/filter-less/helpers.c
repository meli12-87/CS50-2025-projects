...
// Helper function to limit RGB values to 255
int limit(int value)
{
...
void sepia(int height, int width, RGBTRIPLE image[height][width])
{
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            image[i][j].rgbtBlue = limit(sepiaBlue); // fixed typo: avlimit → limit
        }
    }
}
...
void reflect(int height, int width, RGBTRIPLE image[height][width])
{
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width / 2; j++)
        {
            ...
            image[i][width - 1 - j] = temp; // fixed misplaced brace & swap logic
        }
    }
}
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            for (int offseti = -1; offseti <= 1; offseti++)
            {
                ...
                {
                    int neighbori = i + offseti;
                    int neighborj = j + offsetj;
